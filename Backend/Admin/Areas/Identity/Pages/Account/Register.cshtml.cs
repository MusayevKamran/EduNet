    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (!await _roleManager.RoleExistsAsync(UserStatus.ADMIN))
                    {
                        await _roleManager.CreateAsync(new AppRole(UserStatus.ADMIN));
                    }
                    if (!await _roleManager.RoleExistsAsync(UserStatus.TEACHER))
                    {
                        await _roleManager.CreateAsync(new AppRole(UserStatus.TEACHER));
                    }
                    if (!await _roleManager.RoleExistsAsync(UserStatus.USER))
                    {
